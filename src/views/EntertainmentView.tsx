...
const EntertainmentView: React.FC<{ isAdmin: boolean }> = ({ isAdmin }) => {
  ...
  // Khởi tạo trạng thái từ LocalStorage để hiện nội dung ngay lập tức khi mở máy (Offline)
  ...
  // ĐỒNG BỘ REALTIME: Lắng nghe thay đổi từ máy chủ Firebase
  useEffect(() => {
  ...
  // Hàm chuẩn hóa link Youtube để nhúng Iframe
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsEditing(false);
      alert("✅ ĐỒNG BỘ THÀNH CÔNG: Dữ liệu đã được cập nhật cho toàn bộ máy chiến sĩ!");
    } catch (e) {
      console.error("Firebase Error:", e);
      alert("❌ LỖI: Không thể gửi dữ liệu lên máy chủ.");
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-50 p-6 rounded-[3rem] border border-slate-200 shadow-inner">
          ...
          <div className="space-y-3 max-h-[800px] overflow-y-auto pr-2 custom-scrollbar">
            {songs.map((song, i) => (
              <div key={song.id} className="bg-white p-4 rounded-3xl border shadow-sm transition-hover hover:shadow-md">
                ...
                {isEditing ? (
                  <input className="w-full text-[10px] p-2 bg-slate-50 rounded-xl italic border border-slate-200 mt-1" placeholder="Dán link Youtube tại đây..." value={song.embedUrl} onChange={e => setSongs(prev => prev.map(s => s.id === song.id ? {...s, embedUrl: e.target.value} : s))} />
                ) : (
                  song.embedUrl && <iframe title={song.name} src={song.embedUrl} className="w-full aspect-video rounded-2xl mt-2 border shadow-inner" allowFullScreen />
                )}
              </div>
            ))}
          </div>
              ...
              <div key={dance.id} className="bg-white p-6 rounded-[2.5rem] border-2 border-white shadow-lg">
                ...
                {isEditing ? (
                  ...
                  <div className="aspect-video bg-black rounded-[2rem] overflow-hidden shadow-2xl relative">
                    {dance.embedUrl ? <iframe title={dance.name} src={dance.embedUrl} className="w-full h-full" allowFullScreen /> : <div className="h-full flex items-center justify-center text-white/20 font-black italic text-xs">CHƯA CẬP NHẬT VIDEO</div>}
                  </div>
                )}
              </div>
            ))}
